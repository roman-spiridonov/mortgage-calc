var calculator = (function() {
    ...
    function Calculator(option) {
        this._data;
        this._option = option || 3;
        this._isInit = false;
    }


    Calculator.prototype.findn = function(f, Gm, h, w, R, A, B, S) { // deposit
        var fh = f / h;
    ...
    Calculator.prototype.findm = function(f, Gm, h, w, R, A, B, S) { // credit
        if (this._option == 1) {
    ...
    Calculator.prototype.fn = function(x, f, Gm, h, w, R, A, B, S) { // m(n) for arbitrary S
        var fh = f / h;
        if (this._option == 1) { // pay B+R each month
            return Math.log(1 - Gm * (this.Sn(x, f, Gm, h, w, R, A, B, S) - A) / (B + R)) / Math.log(1 / (1 + Gm));
        } else if (this._option == 2) { // pay (B+R)*h each month
            return Math.log(1 - (this.Sn(x, f, Gm, h, w, R, A, B, S) - A) / (B + R) * (1 - h / (1 + Gm))) / Math.log(h / (1 + Gm));
        } else if (this._option == 3) { // w - speed of wages
            return Math.log(1 - (this.Sn(x, f, Gm, h, w, R, A, B, S) - A) / (B + R) * (1 - w / (1 + Gm))) / Math.log(w / (1 + Gm));;
        }
    };

    Calculator.prototype.Sn = function(x, f, Gm, h, w, R, A, B, S) { // S(n) - what cost of property one can afford when saving for n years at given params
        // S(n)
        var fh = f / h;
        if (this._option == 1 || this._option == 2) {
    ...
    Calculator.prototype.calcData = function(f, Gm, h, w, s, R, A, B, S) {
        // Calculate
        var n = this.findn(f, Gm, h, w, R, A, B, S);
        var m = this.findm(f, Gm, h, w, R, A, B, S);
    ...
    Calculator.prototype.init = function(option) {
        this._option = option || 3;
